...
class DraftItemPolicyTest < ActiveSupport::TestCase
  ...
  test 'admin user should be able to do everything' do
    ...
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :create?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :show?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :update?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :destroy?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :set_thumbnail?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :file_create?
    assert_predicate DraftItemPolicy.new(current_user, draft_item), :file_destroy?
  end
