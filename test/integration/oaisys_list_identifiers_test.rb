...
class OaisysListIdentifiersTest < ActionDispatch::IntegrationTest
  ...
  def list_identifiers_item_until_date_xml(just_after_current_time)
    assert_valid_against_schema
    items = Oaisys::Engine.config.oai_dc_model.public_items.updated_before(just_after_current_time)
                          .limit(Oaisys::Engine.config.items_per_request)
                          .belongs_to_path(@community.id)

    theses = Oaisys::Engine.config.oai_etdms_model.public_items.updated_before(just_after_current_time)
                           .limit(Oaisys::Engine.config.items_per_request)
                           .belongs_to_path(@community.id)

    # I believe this should be reversed to match solrs ordering but for some reason a simple .reverse! on it doesn't stick?
    all_identifiers = (items + theses).sort_by!(&:updated_at).pluck(:id, :updated_at, :member_of_paths)

    assert_identifiers_response(all_identifiers)
  end
