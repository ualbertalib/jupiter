...
class Digitization::BatchArtifactSetupIngestTest < ActiveSupport::TestCase
  ...
  test 'valid batch ingest' do
    ...
    assert @batch_artifact_setup_ingest.valid?
  end

  test 'invalid without csvfile' do
    assert_not @batch_artifact_setup_ingest.valid?
    assert_equal("can't be blank", @batch_artifact_setup_ingest.errors[:csvfile].first)
  end
  ...
  test 'invalid without expected headings' do
    ...
    @batch_artifact_setup_ingest.csvfile.attach(io: csv_content,
    ...
    assert_equal(errors, @batch_artifact_setup_ingest.errors[:csvfile])
  end
