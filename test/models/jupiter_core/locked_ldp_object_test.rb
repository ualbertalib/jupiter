...
class LockedLdpObjectTest < ActiveSupport::TestCase
  ...
  test 'solr integration is working' do
    ...
    obj = @@klass.new_locked_ldp_object(title: first_title, creator: creator, owner: users(:user).id,
                                        visibility: 'public')
    ...
    another_obj = @@klass.new_locked_ldp_object(title: second_title, creator: creator, owner: users(:user).id,
                                                visibility: 'public')
