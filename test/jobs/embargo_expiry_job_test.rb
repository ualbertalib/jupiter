...
class EmbargoExpiryJobTest < ActiveJob::TestCase
  ...
  test 'that job transitions only expired item embargos into proper state' do
    ...
    assert_equal not_expired_item.visibility_after_embargo, CONTROLLED_VOCABULARIES[:era][:visibility].public
  ...
  test 'that job transitions expired thesis embargos into proper state' do
    ...
    assert_equal not_expired_thesis.visibility_after_embargo, CONTROLLED_VOCABULARIES[:era][:visibility].public
