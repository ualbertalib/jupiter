...
class Digitization::BatchArtifactIngestJobTest < ActiveJob::TestCase
  ...
  test 'successful artifacts ingestion' do
    ...
    batch_artifact_setup_ingest.csvfile.attach(io: File.open(file_fixture('digitization_artifacts_manifest.csv')),
    ...
    Digitization::BatchArtifactIngestJob.perform_now(batch_artifact_setup_ingest, book.peel_id, book.part_number,
                                                     'dig439b')

    book.reload
    assert(batch_artifact_setup_ingest.completed?)
  ...
  test 'failure because book does not exist' do
    ...
    assert_raises ActiveRecord::RecordNotFound do
      Digitization::BatchArtifactIngestJob.perform_now(batch_artifact_setup_ingest, 1, 1, 'dig439b')
    end
    assert(batch_artifact_setup_ingest.failed?)
  end
  ...
  test 'captures exceptions and updates batch ingest model' do
    ...
    def batch_artifact_setup_ingest.archival_information_package_path
    ...
    assert(batch_artifact_setup_ingest.failed?)
    assert_equal('Testing! Error has happened!', batch_artifact_setup_ingest.error_message)
  end
