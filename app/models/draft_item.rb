class DraftItem < ApplicationRecord
  ...
  # Maps Language names to CONTROLLED_VOCABULARIES[:era][:language] URIs
  def languages_as_uri
    languages.pluck(:name).map do |language|
      CONTROLLED_VOCABULARIES[:era][:language].send(language)
    end
  end

  def languages_for_uris(uris)
    uris.map do |uri|
      code = CONTROLLED_VOCABULARIES[:era][:language].from_uri(uri)
  ...
  # Maps DraftItem.licenses to CONTROLLED_VOCABULARIES[:era][:license]
  def license_as_uri
    ...
    begin
      CONTROLLED_VOCABULARIES[:era][:license].send(code)
    rescue JupiterCore::VocabularyMissingError
      CONTROLLED_VOCABULARIES[:era][:old_license].send(code)
    end
  end
  ...
  def license_for_uri(uri)
    ...
    code = CONTROLLED_VOCABULARIES[:era][:license].from_uri(uri) ||
           CONTROLLED_VOCABULARIES[:era][:old_license].from_uri(uri)
  ...
  # Maps Type names to CONTROLLED_VOCABULARIES[:era][:item_type]
  def item_type_as_uri
    code = ITEM_TYPE_TO_URI_CODE.fetch(type.name.to_sym)
    CONTROLLED_VOCABULARIES[:era][:item_type].send(code)
  end

  def item_type_for_uri(uri, status:)
    code = CONTROLLED_VOCABULARIES[:era][:item_type].from_uri(uri)
    if status.present?
      if status.include?(CONTROLLED_VOCABULARIES[:era][:publication_status].draft)
        name = 'journal_article_draft'
      elsif status.include?(CONTROLLED_VOCABULARIES[:era][:publication_status].published)
  ...
  # Maps DraftItem.visibilities to CONTROLLED_VOCABULARIES[:era][:visibility]
  def visibility_as_uri
    ...
    CONTROLLED_VOCABULARIES[:era][:visibility].send(code)
  end

  def visibility_for_uri(uri)
    code = CONTROLLED_VOCABULARIES[:era][:visibility].from_uri(uri)
  ...
  # Maps DraftItem.visibility_after_embargo to CONTROLLED_VOCABULARIES[:era][:visibility]
