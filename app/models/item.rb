class Item < JupiterCore::Doiable
  ...
  validates :publication_status, compound_uri: { compounds: [
    ...
    item.item_type == CONTROLLED_VOCABULARIES[:era][:item_type].article && item.publication_status.present?
  }
  ...
  def item_type_with_status_code
    ...
    item_type_code = CONTROLLED_VOCABULARIES[:era][:item_type].from_uri(item_type)
    return item_type_code unless item_type_code == :article
    return nil if publication_status.blank?

    publication_status_code = CONTROLLED_VOCABULARIES[:era][:publication_status].from_uri(publication_status.first)
  ...
  def self.from_draft(draft_item)
    ...
    if draft_item.visibility_as_uri == CONTROLLED_VOCABULARIES[:era][:visibility].embargo
      ...
      item.add_to_embargo_history if item.visibility == CONTROLLED_VOCABULARIES[:era][:visibility].embargo
