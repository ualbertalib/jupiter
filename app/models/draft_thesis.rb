class DraftThesis < ApplicationRecord
  ...
  # Maps Language name to CONTROLLED_VOCABULARIES[:era][:language] URI
  def language_as_uri
    return nil if language&.name.blank?

    CONTROLLED_VOCABULARIES[:era][:language].send(language.name)
  end
  ...
  # Maps institution names to CONTROLLED_VOCABULARIES[:era][:institution]
  def institution_as_uri
    return nil if institution&.name.blank?

    CONTROLLED_VOCABULARIES[:era][:institution].send(institution.name)
  end

  def institution_for_uri(uri)
    return nil if uri.blank?

    code = CONTROLLED_VOCABULARIES[:era][:institution].from_uri(uri)
