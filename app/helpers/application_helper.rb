module ApplicationHelper
  ...
  def search_link_for(object, attribute, value: nil, facet: :facet, display: nil)
    ...
    if facet == :range_facet
      link_to(display, search_path(ranges: object.solr_exporter_class.facet_term_for(attribute, value, role: :range_facet)),
              rel: 'nofollow')
    elsif facet == :facet
      link_to(display, search_path(facets: object.solr_exporter_class.facet_term_for(attribute, value)), rel: 'nofollow')
    else
      link_to(display, search_path(search: object.solr_exporter_class.search_term_for(attribute, value)), rel: 'nofollow')
    end
  end
