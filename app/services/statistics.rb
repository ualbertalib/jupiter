class Statistics
  ...
  def self.views_for(item_id:)
    Jupiter::Redis.current.get(counter_key_for(:view, item_id)).to_i || 0
  end

  def self.downloads_for(item_id:)
    Jupiter::Redis.current.get(counter_key_for(:download, item_id)).to_i || 0
  end
  ...
  class << self
    ...
    def increment_action_counter(action:, id:, ip:)
      ...
      is_new_visit = Jupiter::Redis.current.pfadd(uniques_filter_key, ip)

      # ip filters reset at the top of the hour, so if the key was freshly (re-)created we need to set its TTL
      Jupiter::Redis.current.expireat(uniques_filter_key, Time.current.end_of_hour.to_i)
      return unless is_new_visit

      Jupiter::Redis.current.incr(counter_key)
    end
